...
See [`ARCHITECTURE.md`](./ARCHITECTURE.md) for the full spec, schema contract, and the mapping
...
Note `NEXT_PUBLIC_API_URL` must be passed at **build** time (`--build-arg`),
not as a runtime `-e` — see ARCHITECTURE.md "Footguns actually hit" for why.
...
rules (enum allowlists, date format, note-overflow, multi-value dedup — see
`ARCHITECTURE.md` "Invariants"). Responses are constrained via OpenAI's structured-output
JSON schema mode.
